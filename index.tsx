...
const ToastProvider: React.FC<React.PropsWithChildren<{}>> = ({ children }) => {
    ...
    return (
        <ToastContext.Provider value={{ addToast }}>
            {children}
            <div className="fixed top-5 right-5 z-[100] w-full max-w-xs sm:max-w-sm space-y-3 pr-safe-right pt-safe-top">
                {toasts.map(toast => (
...
class MobileOptimizationManager {
    ...
    private detectMobileFeatures() {
        // 햅틱 피드백 지원 확인
        this.hapticEnabled = 'vibrate' in navigator;
        
        // 제스처 지원 확인
        this.gestureEnabled = 'ontouchstart' in window;
        
        console.log(`📱 모바일 기능 감지: 햅틱=${this.hapticEnabled}, 제스처=${this.gestureEnabled}`);
    }
    ...
    public getScreenOrientation(): 'portrait' | 'landscape' {
    ...
    public forceUnlockAudio(): void {
        if (!this.audioContextUnlocked) {
            try {
                ...
                speechSynthesis.cancel();
                this.audioContextUnlocked = true;
                console.log('🔊 오디오 컨텍스트 강제 활성화됨');
            } catch (error) {
                console.error('오디오 컨텍스트 강제 활성화 실패:', error);
            }
        }
    }
}
...
// v6.0.0 Enhanced Mobile-Optimized TTS System
const speak = (text: string, lang = 'en-US') => {
    ...
    const mobileManager = MobileOptimizationManager.getInstance();
    
    // 모바일에서는 항상 사용자 터치로 오디오 활성화
    if (mobileManager.isMobileDevice()) {
        // 먼저 오디오 컨텍스트 강제 활성화
        mobileManager.forceUnlockAudio();
        
        // 즉시 발음 시도
        executeSpeech(text, lang);
        
        // 만약 실패하면 터치 프롬프트 표시
        setTimeout(() => {
            if (!speechSynthesis.speaking) {
                showMobileTouchPrompt(text, lang);
            }
        }, 100);
...
// 모바일 터치 프롬프트 표시
const showMobileTouchPrompt = (text: string, lang: string) => {
    const touchPrompt = document.createElement('div');
    touchPrompt.className = 'mobile-touch-prompt';
    touchPrompt.style.cssText = `
        position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);
        background: linear-gradient(135deg, rgba(0,0,0,0.95), rgba(30,30,30,0.95));
        color: white; padding: 24px; border-radius: 16px; z-index: 10000;
        text-align: center; font-family: system-ui; 
        box-shadow: 0 20px 40px rgba(0,0,0,0.7);
        backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.1);
        max-width: 300px; animation: fadeInScale 0.3s ease-out;
    `;
    
    touchPrompt.innerHTML = `
        <div style="font-size: 32px; margin-bottom: 12px;">🔊</div>
        <div style="font-size: 18px; font-weight: 600; margin-bottom: 8px;">발음 재생</div>
        <div style="font-size: 14px; opacity: 0.8; line-height: 1.4;">화면을 터치하여<br/>발음을 들어보세요</div>
    `;
    ...
    document.body.appendChild(touchPrompt);
    ...
    const enableAudio = () => {
        ...
        // 프롬프트 제거
        if (document.body.contains(touchPrompt)) {
            document.body.removeChild(touchPrompt);
        }
...
// 실제 TTS 실행 함수
const executeSpeech = (text: string, lang = 'en-US') => {
    console.log(`🔊 ExecuteSpeech called with: "${text}"`);
    
    // 기존 음성 완전히 중지
    speechSynthesis.cancel();
    
    // 짧은 지연 후 실행 (모바일 브라우저 호환성)
    setTimeout(() => {
        const utterance = new SpeechSynthesisUtterance(text);
        utterance.lang = lang;
        
        // 개선된 발음 설정 - 더 자연스럽고 명확한 발음
        utterance.rate = 0.7;    // 더 느리게 (명확한 발음을 위해)
        utterance.pitch = 1.0;   // 자연스러운 피치 유지
        utterance.volume = 0.9;  // 약간 낮춘 볼륨 (더 자연스럽게)
        ...
        utterance.onstart = () => {
            ...
            if (mobileManager.isMobileDevice()) {
                mobileManager.triggerHapticFeedback('light');
            }
        };
        ...
        utterance.onerror = (event) => {
            ...
            // 모바일에서 특정 오류 시 자동 재시도
            if (mobileManager.isMobileDevice() && 
                (event.error === 'network' || event.error === 'interrupted' || event.error === 'canceled')) {
        ...
        const selectVoiceAndSpeak = () => {
            ...
            // 개선된 음성 선택 로직 - 더 자연스러운 음성 우선
            const preferredVoices = [
                // 최고 품질 네이티브 음성 (iOS/macOS)
                'Samantha', 'Alex', 'Victoria', 'Allison', 'Ava', 'Susan', 'Zoe',
                // 고품질 Google 음성 (Android/Chrome)
                'Google US English', 'Google UK English Female', 'Google UK English Male',
                'Google français', 'Google Deutsch',
                // Microsoft 음성 (Windows/Edge)
                'Microsoft Aria Online (Natural) - English (United States)',
                'Microsoft Jenny Online (Natural) - English (United States)',
                'Microsoft Guy Online (Natural) - English (United States)',
                'Microsoft Zira', 'Microsoft David', 'Microsoft Aria', 'Microsoft Mark',
                // 기타 고품질 음성
                'English (United States)', 'English (United Kingdom)',
                'en-US-language', 'en-GB-language',
                // 기본 영어 음성
                'English', 'en-US', 'en-GB', 'US English', 'UK English'
            ];
            ...
            if (lang.startsWith('en')) {
                ...
                for (const voiceName of preferredVoices) {
                    ...
                    if (bestVoice) {
                        console.log(`🔊 Found exact preferred voice: ${bestVoice.name}`);
                        break;
                    }
                }
                ...
                if (!bestVoice) {
                    for (const voiceName of preferredVoices) {
                        ...
                        if (bestVoice) {
                            console.log(`🔊 Found partial match preferred voice: ${bestVoice.name}`);
                            break;
                        }
                    }
                }
                
                // 3. 로컬 영어 음성 우선 (네이티브 품질)
                if (!bestVoice) {
                    bestVoice = voices.find(voice => 
                        voice.lang.startsWith('en') && 
                    ...
                    if (bestVoice) {
                        console.log(`🔊 Found local voice: ${bestVoice.name}`);
                    }
                }
                ...
                if (!bestVoice) {
                    ...
                    if (bestVoice) {
                        console.log(`🔊 Found online/natural voice: ${bestVoice.name}`);
                    }
                }
                ...
                if (!bestVoice) {
                    ...
                    if (bestVoice) {
                        console.log(`🔊 Found US English voice: ${bestVoice.name}`);
                    }
                }
                ...
                if (!bestVoice) {
                    ...
                    if (bestVoice) {
                        console.log(`🔊 Found any English voice: ${bestVoice.name}`);
                    }
                }
            }
            ...
            if (bestVoice) {
                ...
                // 음성별 최적화 설정
                if (bestVoice.name.includes('Samantha') || bestVoice.name.includes('Alex')) {
                    // iOS 네이티브 음성 - 최고 품질
                    utterance.rate = 0.75;
                    utterance.pitch = 1.0;
                } else if (bestVoice.name.includes('Google')) {
                    // Google 음성 - 약간 빠르게
                    utterance.rate = 0.8;
                    utterance.pitch = 1.0;
                } else if (bestVoice.name.includes('Microsoft')) {
                    // Microsoft 음성 - 표준 설정
                    utterance.rate = 0.7;
                    utterance.pitch = 1.0;
                } else {
        ...
        if (speechSynthesis.getVoices().length === 0) {
            ...
            setTimeout(() => {
                if (speechSynthesis.getVoices().length === 0) {
                    console.log('🔊 Timeout reached, using default voice');
                    speechSynthesis.speak(utterance);
                }
            }, 1500);
        } else {
...
const LearnWordsScreen: React.FC<LearnWordsScreenProps> = ({ userSettings, onNavigate, words, wordStats, onWordLearned, addToast, setGlobalLoading }) => {
    ...
    const handleNextWord = () => {
        ...
        console.log(`🔄 다음 단어로 이동 - 현재 단어 학습 완료 처리:`, {
            currentWord: currentWord.term,
            wordId: currentWord.id,
            isQuickReview: isQuickReviewActive,
            currentIndex,
            totalWords: currentWordsSet.length
        });
        ...
        if (nextIndex < currentWordsSet.length) {
            console.log(`➡️ 다음 단어로 이동: ${currentIndex + 1}/${currentWordsSet.length}`);
    ...
    return (
        <div className="p-3 sm:p-8 flex flex-col items-center min-h-screen bg-slate-900">
            {/* 모바일 최적화 헤더 */}
                    ...
                    <div className="flex sm:hidden items-center space-x-2">
                        <div className="w-16 bg-slate-700 rounded-full h-2">
                            <div 
                                ...
                                style={{ width: `${((currentIndex + 1) / currentWordsSet.length) * 100}%` }}
                            ></div>
                        </div>
                        <span className="text-xs text-slate-400">{Math.round(((currentIndex + 1) / currentWordsSet.length) * 100)}%</span>
                    </div>
                </div>
            </div>

            {/* 메인 카드 */}
            <div className="w-full max-w-2xl bg-slate-800 rounded-xl shadow-2xl p-4 sm:p-8 mb-6">
                {/* 단어 표시 영역 */}
                <div className="text-center mb-6">
                    <div className="flex items-center justify-center mb-4">
                        <h2 className="text-3xl sm:text-4xl font-bold text-white mr-3">{currentWord.term}</h2>
                        <button onClick={() => {
                            console.log(`🔊 학습 화면 발음 클릭: ${currentWord.term}`);
                            const mobileManager = MobileOptimizationManager.getInstance();
                            
                            // 즉시 햅틱 피드백
                            if (mobileManager.isMobileDevice()) {
                                mobileManager.triggerHapticFeedback('medium');
                            }
                        ...
                        }} className="text-slate-400 hover:text-cyan-400 text-2xl sm:text-3xl transition-all duration-200 p-2 rounded-lg hover:bg-slate-700/50 active:scale-95" aria-label="단어 발음 듣기">
                            🔊
                        </button>
                    </div>
                    {currentWord.pronunciation && (
                        <p className="text-lg sm:text-xl text-cyan-300 mb-4">[{currentWord.pronunciation}]</p>
                    )}
                    <p className="text-slate-400 text-base sm:text-lg mb-6">{currentWord.partOfSpeech}</p>
                </div>
                ...
                <div className="text-center mb-6">
                    <button
                        ...
                        className="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-semibold rounded-lg shadow-lg transition-all duration-200 active:scale-95"
                    >
                ...
                {showMeaning && (
                    <div className="space-y-4 animate-fadeIn">
                        <div className="bg-slate-700 rounded-lg p-4">
                            <h3 className="text-cyan-300 font-semibold mb-2">의미</h3>
                            <p className="text-white text-base sm:text-lg">{currentWord.meaning}</p>
                        </div>
                        
                        <div className="bg-slate-700 rounded-lg p-4">
                            <h3 className="text-cyan-300 font-semibold mb-2">예문</h3>
                            <p className="text-white text-base sm:text-lg mb-2">{currentWord.exampleSentence}</p>
                            {currentWord.exampleSentenceMeaning && (
                                <p className="text-slate-300 text-sm sm:text-base italic">{currentWord.exampleSentenceMeaning}</p>
                            )}
                        </div>
                        ...
                        <div className="bg-slate-700 rounded-lg p-4">
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="text-cyan-300 font-semibold">AI 예문</h3>
                                <button
                                    onClick={handleGenerateAiExample}
                                    disabled={isFetchingAiExample}
                                    className="px-3 py-1 bg-purple-600 hover:bg-purple-700 disabled:bg-slate-600 text-white text-sm rounded-md transition-colors active:scale-95"
                                >
                                    {isFetchingAiExample ? '생성 중...' : '새 예문'}
                                </button>
                            </div>
                            ...
                            {isFetchingAiExample && (
                                <div className="flex items-center space-x-2 text-slate-400">
                                    ...
                                    <span className="text-sm">AI가 새로운 예문을 생성하고 있습니다...</span>
                                </div>
                            )}
                            ...
                            {aiExample && (
                                <div className="space-y-2">
                                    <div className="flex items-start space-x-2">
                                        <p className="text-white text-base flex-1">{aiExample.newExampleSentence}</p>
                                        <button onClick={() => {
                                            console.log(`🔊 AI 예문 발음 클릭: ${aiExample.newExampleSentence}`);
                                            const mobileManager = MobileOptimizationManager.getInstance();
                                            ...
                                            speak(aiExample.newExampleSentence);
                                        }} className="text-slate-400 hover:text-cyan-400 text-lg transition-all duration-200 p-1 rounded-md hover:bg-slate-600/50 active:scale-95" aria-label="AI 예문 발음 듣기">🔊</button>
                                    </div>
                                    <p className="text-slate-300 text-sm italic">{aiExample.newExampleSentenceMeaning}</p>
                                </div>
                            )}
                            
                            {!aiExample && !isFetchingAiExample && (
                                <p className="text-slate-400 text-sm">AI 예문을 생성하려면 '새 예문' 버튼을 클릭하세요.</p>
                            )}
                        </div>
                        ...
                        <div className="bg-slate-700 rounded-lg p-4">
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="text-cyan-300 font-semibold">AI 이미지</h3>
                                <button
                                    onClick={handleGenerateAiImage}
                                    disabled={isFetchingAiImage}
                                    className="px-3 py-1 bg-green-600 hover:bg-green-700 disabled:bg-slate-600 text-white text-sm rounded-md transition-colors active:scale-95"
                                >
                                    {isFetchingAiImage ? '생성 중...' : '이미지 생성'}
                                </button>
                            </div>
                            ...
                            {isFetchingAiImage && (
                                <div className="flex items-center space-x-2 text-slate-400">
                                    ...
                                    <span className="text-sm">AI가 단어와 관련된 이미지를 생성하고 있습니다...</span>
                                </div>
                            )}
                            ...
                            {aiGeneratedImage && (
                                <div className="mt-3">
                                    <img 
                                        ...
                                        alt={`AI generated image for ${currentWord.term}`}
                                        className="w-full max-w-sm mx-auto rounded-lg shadow-md"
                                    />
                                </div>
                            )}
                            
                            {!aiGeneratedImage && !isFetchingAiImage && (
                                <p className="text-slate-400 text-sm">단어와 관련된 AI 이미지를 생성하려면 '이미지 생성' 버튼을 클릭하세요.</p>
                            )}
                        </div>
                    </div>
                )}
            ...
            {/* 모바일 최적화 버튼 영역 */}
            <div className="w-full max-w-2xl flex flex-col sm:flex-row gap-3 sm:gap-4">
                {!showMeaning ? (
                    <button
                        onClick={() => setShowMeaning(true)}
                        className="flex-1 py-4 sm:py-3 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-semibold rounded-lg shadow-lg transition-all duration-200 active:scale-95 text-lg sm:text-base"
                    >
                        의미 보기
                    </button>
                ) : (
                    <button
                        ...
                        className="flex-1 py-4 sm:py-3 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white font-semibold rounded-lg shadow-lg transition-all duration-200 active:scale-95 text-lg sm:text-base"
                    >
                        다음 단어
                    </button>
                )}
                ...
                <button
                    onClick={() => onNavigate('dashboard')}
                    className="py-3 px-6 bg-slate-600 hover:bg-slate-500 text-white font-semibold rounded-lg shadow-lg transition-all duration-200 active:scale-95"
                >
                    홈으로
                </button>
            </div>
                                ...
                                <div className="mt-2 pt-2 border-t border-slate-600 animate-fadeIn">
                                    <p className="text-teal-300 font-semibold mb-1">✨ AI 추가 예문:</p>
                                     <button onClick={() => {
                                        const mobileManager = MobileOptimizationManager.getInstance();
                                        mobileManager.triggerHapticFeedback('light');
                                        speak(aiReviewExample.newExampleSentence);
                                    }} className="text-slate-400 hover:text-cyan-400 text-lg mr-1" aria-label="AI 예문 발음 듣기">🔊</button>
            ...
            <div className="w-full max-w-xl bg-slate-700 rounded-xl shadow-2xl p-6 sm:p-8">
                <div className="text-center mb-6">
                    ...
                    <div className="flex items-center justify-center">
                        ...
                        <button onClick={() => {
                            console.log(`🔊 퀴즈 발음 클릭: ${currentWord.term}`);
                            const mobileManager = MobileOptimizationManager.getInstance();
                            ...
                            if (button) {
                                button.style.transform = 'scale(0.9)';
                                button.style.backgroundColor = 'rgba(6, 182, 212, 0.3)';
                                setTimeout(() => {
                            ...
                            speak(currentWord.term);
                        }} className="text-slate-400 hover:text-cyan-400 text-2xl p-1 rounded transition-all duration-200 hover:bg-slate-600/50 active:scale-90 focus:outline-none focus:ring-2 focus:ring-cyan-400" aria-label="단어 발음 듣기">
                            🔊
                        </button>
                    </div>
                        ...
                        <li key={word.id} className={`p-4 rounded-lg shadow transition-colors ${word.stat.isMastered ? 'bg-slate-700/70 hover:bg-slate-600/70' : 'bg-slate-700 hover:bg-slate-600'}`}>
                            <div className="flex justify-between items-start">
                                ...
                                <div className="flex flex-col sm:flex-row space-y-1 sm:space-y-0 sm:space-x-1 flex-shrink-0 ml-2 items-end">
                                     <button onClick={() => {
                                        console.log(`🔊 All Words 발음 클릭: ${word.term}`);
                                        const mobileManager = MobileOptimizationManager.getInstance();
                                        ...
                                        speak(word.term);
                                    }} className="text-slate-400 hover:text-cyan-400 text-xl p-1.5 rounded-md hover:bg-slate-500 transition-all duration-200 active:scale-90 focus:outline-none focus:ring-2 focus:ring-cyan-400" aria-label={`${word.term} 발음 듣기`}>
                                        🔊
                                    </button>
                                    <button 
...
const ManageWordsScreen: React.FC<ManageWordsScreenProps> = ({ userSettings, onNavigate, myWords, allWords, wordStats, onSaveCustomWord, onDeleteCustomWord, updateWordStat, addToast, setGlobalLoading }) => {
    ...
    const processAndAddExtractedWords = async (wordsToProcess: string[]) => {
        ...
        // v2.8.0: 자동 저장 완료 시 처리된 단어들 제거 - 전체 단어 목록에 저장 완료
        if (userSettings.autoSaveExtractedWords && successCount > 0) {
            ...
            setProcessingLog(prev => [...prev, `💾 전체 단어 목록 저장 완료: ${successCount}개 단어가 전체 단어 목록에 추가되고 추출 목록에서 제거됨`]);
        }
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            // v3.0.0: 향상된 자동 저장 기능 - 추출된 단어를 전체 단어에 저장
            if (userSettings.autoSaveExtractedWords && newWords.length > 0) {
                setProcessingLog(prev => [...prev, `💾 v3.0.0 자동 저장 기능 활성화: ${newWords.length}개 단어를 전체 단어 목록에 자동 저장 시작...`]);
                ...
                addToast(`🚀 추출된 단어를 전체 단어에 저장: ${wordsToAutoSave.length}개 단어가 AI로 자동 처리되어 전체 단어 목록에 추가됩니다!`, 'success');
                ...
                addToast(`💡 팁: 설정에서 "자동 저장" 기능을 활성화하면 파일에서 추출된 단어들이 자동으로 전체 단어 목록에 저장됩니다!`, 'info');
            }
    ...
    // 🚀 고급 배치 처리 함수 - 추출된 단어를 전체 단어 목록에 저장
    const handleStartBatchProcessing = async () => {
        if (selectedWords.size === 0) {
            addToast("전체 단어 목록에 저장할 단어를 선택해주세요.", "warning");
            return;
        }
        ...
        setProcessingLog(prev => [
            ...prev,
            `🚀 전체 단어 목록 저장 시작: ${wordsArray.length}개 단어를 전체 단어 목록에 추가`,
        ...
        try {
            const processedWords = await processBatchWords(
                ...
                (current, total, word) => {
                    ...
                    setProcessingLog(prev => [
                        ...
                        `🔄 전체 단어 목록에 저장 중: ${word} (${current + 1}/${total})`
                    ]);
                }
            );

            // 처리된 단어들을 실제로 전체 단어 목록에 저장
            let successCount = 0;
            ...
            setProcessingLog(prev => [
                ...prev,
                `✅ 전체 단어 목록 저장 완료!`,
                `📊 최종 결과: ${successCount}개 단어가 전체 단어 목록에 성공적으로 추가됨, ${processedWords.length - successCount}개 실패`
            ]);

            addToast(`전체 단어 목록 저장 완료: ${successCount}개 단어가 전체 단어 목록에 추가되었습니다.`, 'success');
            ...
            console.error('Batch processing error:', error);
            addToast('전체 단어 목록 저장 중 오류가 발생했습니다.', 'error');
            setProcessingLog(prev => [...prev, `❌ 전체 단어 목록 저장 오류: ${error instanceof Error ? error.message : '알 수 없는 오류'}`]);
        } finally {
            setIsProcessingFileWords(false);
            setBatchProcessingProgress({ current: 0, total: 0, word: '' });
        }
                    ...
                    {extractedWords.length > 0 && (
                        <div className="bg-slate-700 p-4 rounded-lg">
                            ...
                            <div className="mt-4 pt-4 border-t border-slate-600">
                                ...
                                >
                                    <span className="mr-1">📚</span>
                                    전체 단어 목록에 전체 저장 ({extractedWords.length}개)
                                </button>
                                    </div>
                                ...
                                <div className="flex flex-col sm:flex-row gap-3">
                                    ...
                                    >
                                        ...
                                        {isProcessingFileWords ? '전체 단어 목록에 저장 중...' : `선택된 ${selectedWords.size}개 단어를 전체 단어 목록에 저장`}
                                    </button>
                ...
                <div className="flex justify-center mb-8">
                    ...
                    >
                        ...
                        <div className={`absolute inset-0 w-full h-full bg-gradient-to-br from-slate-700 to-slate-800 rounded-xl shadow-2xl border border-slate-600 flex flex-col justify-center items-center p-8 backface-hidden ${
                            ...
                            <div className="text-center">
                                <div className="flex items-center justify-center mb-4">
                                    <h2 className="text-4xl font-bold text-white mr-3">{currentWord?.term}</h2>
                                    <button onClick={() => {
                                        console.log(`🔊 플래시카드 발음 클릭: ${currentWord?.term}`);
                                        const mobileManager = MobileOptimizationManager.getInstance();
                                    ...
                                    }} className="text-slate-300 hover:text-cyan-400 text-2xl p-2 rounded transition-all duration-200 hover:bg-slate-600/50 active:scale-90 focus:outline-none focus:ring-2 focus:ring-cyan-400" aria-label="단어 발음 듣기">
                                        🔊
                                    </button>
                                </div>
                                {currentWord?.pronunciation && (
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        if (userSettings) {
            ...
            const today = getTodayDateString();
            let count = 0;
            console.log(`📊 오늘 학습한 단어 수 계산 시작 (학년: ${userSettings.grade})`);
            ...
            Object.values(wordStats).forEach(stat => {
                ...
                if (word && userSettings && word.gradeLevel === userSettings.grade && stat.lastReviewed?.startsWith(today)) {
                    count++;
                    console.log(`✅ 오늘 학습한 단어 발견: ${word.term} (${word.gradeLevel})`);
                }
            });
            
            console.log(`📊 오늘 학습한 단어 총 개수: ${count}`);
    ...
    const handleSaveSettings = (newSettings: UserSettings) => {
        ...
        console.log(`⚙️ 설정 변경으로 인한 학습 진행도 재계산 (이전: ${userSettings?.grade} → 현재: ${newSettings.grade})`);
        const today = getTodayDateString();
        let count = 0;
        Object.values(wordStats).forEach(stat => {
            const word = sampleWords.find(w => w.id === stat.id) || myWords.find(w => w.id === stat.id);
            ...
            if (word && newSettings && word.gradeLevel === newSettings.grade && stat.lastReviewed?.startsWith(today)) {
                count++;
                console.log(`✅ 새 학년에 맞는 오늘 학습한 단어: ${word.term} (${word.gradeLevel})`);
            }
        });
        console.log(`📊 설정 변경 후 오늘 학습한 단어 총 개수: ${count}`);
        setLearnedWordsTodayCount(count);
    };
    ...
    const handleWordLearned = useCallback((wordId: string | number, isQuickReview: boolean = false) => {
        ...
        console.log(`📚 단어 학습 처리 시작:`, {
            wordId,
            isQuickReview,
            today,
            currentStatBeforeUpdate,
            userGrade: userSettings?.grade
        });
        ...
        if (!isQuickReview && userSettings) {
             ...
             if(word && word.gradeLevel === userSettings.grade) {
                ...
                const wasAlreadyReviewedToday = currentStatBeforeUpdate.lastReviewed && currentStatBeforeUpdate.lastReviewed.startsWith(today);
                console.log(`📊 학습 진행도 업데이트 체크:`, {
                    wasAlreadyReviewedToday,
                    lastReviewed: currentStatBeforeUpdate.lastReviewed,
                    willIncrement: !wasAlreadyReviewedToday
                });
                
                if (!wasAlreadyReviewedToday) { 
                     setLearnedWordsTodayCount(prev => {
                         const newCount = prev + 1;
                         console.log(`✅ 학습 진행도 업데이트: ${prev} → ${newCount}`);
                         return newCount;
                     });
                } else {
                    console.log(`⏭️ 이미 오늘 학습한 단어라서 진행도 숫자 업데이트 안함`);
                }
             } else {
                console.log(`❌ 단어 학습 진행도 업데이트 안함:`, {
                    wordFound: !!word,
                    wordGrade: word?.gradeLevel,
                    userGrade: userSettings.grade,
                    gradeMatch: word?.gradeLevel === userSettings.grade
                });
             }
        } else {
            console.log(`⏭️ 빠른 복습이거나 사용자 설정 없음으로 진행도 업데이트 안함`);
        }
